...
public class Question_1 {
	...
	public static void main(String[] args) throws Exception {
		...
		if (otherArgs.length !=3 ) {
			System.err.println("Usage: Question_1 <path_to_business.csv> <path_to_review.csv> <output path>");
			System.exit(2);
		}
	...
	public static class Map_B extends Mapper<LongWritable, Text, Text, Text>{
		...
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			context.write(new Text(businessData[0]),new Text("business^"+businessData[1]+ "^" +businessData[2]));
	...
	public static class Map_R extends Mapper<LongWritable, Text, Text, Text>{
		...
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			String delims = "^";
			String[] businessData = StringUtils.split(value.toString(),delims);
			context.write(new Text(businessData[2]),new Text("review^"+businessData[3]));
		}
	}
		...
		public void reduce(Text key, Iterable<Text> values, Context context ) throws IOException, InterruptedException {
			...
			for (Text val : values) {
				...
				if(value[0].compareTo("business")==0){
					business_HashMap.put(key.toString(), value[1] + "^" + value[2]);
				}
		...
		protected void cleanup(Context context) throws IOException,	InterruptedException {
			...
			for(String businessID : sorted_map.keySet()) {
				if(business_HashMap.containsKey(businessID)){
					String addressAndCategoriesData=business_HashMap.get(businessID);
					count++;
					String delims = "^";
					String[] data = StringUtils.split(addressAndCategoriesData.toString(),delims);
					if(count>10)
						break;
					context.write(new Text(businessID),new Text(data[0] + data[1] +"\t" + review_HashMap.get(businessID).toString()));
				}
			}
		}
	}
